        ...
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 mb-8 transition-all hover:shadow-md">
            <form method="GET" action="{{ route('admin.jadwal.index') }}" class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
                ...
                <div class="lg:col-span-3">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[2px] mb-3 ml-1">Cari Mata Pelajaran</label>
                ...
                <div class="lg:col-span-3">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[2px] mb-3 ml-1">Filter Guru</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-300">
                            <i data-lucide="users" class="w-5 h-5"></i>
                        </div>
                        <select name="guru_id" class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl pl-12 pr-10 py-3.5 appearance-none focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 focus:bg-white transition-all text-sm font-semibold text-slate-600">
                ...
                <div class="lg:col-span-3">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[2px] mb-3 ml-1">Filter Hari</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-300">
                            <i data-lucide="calendar" class="w-5 h-5"></i>
                        </div>
                        <select name="hari" class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl pl-12 pr-10 py-3.5 appearance-none focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 focus:bg-white transition-all text-sm font-semibold text-slate-600">
                ...
                <div class="lg:col-span-3 flex gap-3">
                    <button type="submit" class="flex-1 bg-slate-900 text-white font-bold py-4 rounded-2xl hover:bg-black transition-all shadow-lg flex items-center justify-center gap-2 text-xs uppercase tracking-widest">
                        <i data-lucide="filter" class="w-4 h-4 text-orange-400"></i>
                        Filter
                    </button>

                    @if(request('search') || request('guru_id') || request('hari'))
                        ...
                        <tr class="hover:bg-orange-50/30 transition-all group">
                            <td class="px-8 py-6">
                                <div class="flex items-center">
                                    <div class="relative">
                                        ...
                                        @if($jadwal->isNow())
                                            <span class="absolute -top-1 -right-1 flex h-4 w-4 mr-4">
                                    ...
                                    <div>
                                        ...
                                        @if($jadwal->isNow())
                                            <span class="text-[9px] text-green-600 font-black uppercase tracking-widest animate-pulse">Sedang Mengajar</span>
                                        @else
                            ...
                            <td class="px-8 py-6">
                                @php
                                    ...
                                    if(str_contains($jadwal->ruangan,'MPLB')) $badge='bg-rose-100 text-rose-600';
                                    elseif(str_contains($jadwal->ruangan,'PPLG')) $badge='bg-emerald-100 text-emerald-600';
                                    elseif(str_contains($jadwal->ruangan,'AKKUL')) $badge='bg-sky-100 text-sky-600';
                                    elseif(str_contains($jadwal->ruangan,'TJKT')) $badge='bg-amber-100 text-amber-700';
                                @endphp
                                <span class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-tight {{ $badge }}">
                                    {{ $jadwal->ruangan ?? 'Ruang Umum' }}
                                </span>
                            </td>
                            <td class="px-8 py-6">
                                <div class="flex justify-center gap-3">
                                    ...
                                    <form action="{{ route('admin.jadwal.destroy', $jadwal->id) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" onclick="return confirm('Hapus jadwal ini?')" class="h-11 w-11 flex items-center justify-center text-red-500 bg-red-50 hover:bg-red-500 hover:text-white rounded-2xl transition-all shadow-sm">
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-24 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-slate-400 font-black uppercase tracking-[3px] text-xs">Data Kosong</h3>
                                    <p class="text-slate-300 text-sm mt-2 font-medium">Tidak ada jadwal yang sesuai filter</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
