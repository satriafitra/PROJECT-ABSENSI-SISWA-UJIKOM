    ...
    <div class="glass-panel rounded-[1.5rem] shadow-xl shadow-slate-200/50 overflow-hidden">
        ...
        <div class="px-5 py-4 bg-white/50 flex flex-wrap items-center justify-between gap-3 border-b border-slate-100">
            ...
            <div class="flex items-center gap-2">
                {{-- Tombol Excel --}}
                <a href="{{ route('admin.rekap.excel', ['date' => request('date'), 'search' => request('search')]) }}"
                    class="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-xl text-xs font-semibold hover:bg-emerald-700 transition-all shadow-sm">
                    <i data-lucide="file-spreadsheet" class="w-3.5 h-3.5 text-white"></i> Excel
                </a>

                {{-- Tombol PDF --}}
                <a href="{{ route('admin.rekap.pdf', ['date' => request('date'), 'search' => request('search')]) }}"
                    class="flex items-center gap-1.5 px-4 py-2 bg-slate-800 text-white rounded-xl text-xs font-semibold hover:bg-slate-700 transition-all shadow-sm">
                    ...
                    <tr class="row-hover">
                        ...
                        <td class="px-5 py-4">
                            <div class="text-[10px] text-slate-500 italic max-w-[150px] truncate">
                                {{-- Mengambil 'notes' atau 'keterangan' dari DB --}}
                                {{ $item->notes ?? $item->keterangan ?? 'Tanpa keterangan' }}
                            </div>
                        </td>
